...
void ConfigLoad()
{
	...
	if(!hLR_ExWeapons.ImportFromFile(sPath) || !hLR_ExWeapons.GotoFirstSubKey())
	{
	...
	if(hLR_ExWeapons.JumpToKey("WeaponsList"))
	{
		...
		do
		{
			...
			g_fWeaponsCoeff[g_iWeaponsBlocksCount] = hLR_ExWeapons.GetFloat("coefficient", 1.0);
			g_iWeaponsBlocksCount++;
		}
		while(hLR_ExWeapons.GotoNextKey() && g_iWeaponsBlocksCount < 47);
	}
	else SetFailState("[" ... PLUGIN_NAME ... " ExWeapons] section WeaponsList is not found (%s)", sPath);
...
public void LR_OnPlayerKilled(Event hEvent, int& iExpGive)
{
	...
	GetEventString(hEvent, "weapon", sBuffer, 48);
	FormatEx(sClassname, 64, "weapon_%s", sBuffer);

	if(sBuffer[0] == 'k' || sBuffer[0] == 'b')
	{
	...
	for(int i = 0; i < 47; i++)
	{
		if(StrEqual(sClassname, g_sWeaponClassname[i]))
		{
			if(g_bWeaponsCoeffActive)
			{
				iExpGive = RoundToNearest(iExpGive * g_fWeaponsCoeff[i]);
			}
...
void PrintTopWeapons(int iClient, int iWeaponId, char sWeaponClassname[192])
{
	if(LR_GetClientStatus(iClient))
	{
		char sQuery[512];
		...
		hDataPack.WriteCell(iWeaponId);
		g_hDatabase.Query(SQL_PrintTopWeapons, sQuery, hDataPack);
	}
}

public void SQL_PrintTopWeapons(Database db, DBResultSet dbRs, const char[] sError, any hData)
{
	if(!dbRs)
	{
	...
	DataPack hDataPack = hData;
	hDataPack.Reset();
	int iClient = hDataPack.ReadCell();
	int iWeaponId = hDataPack.ReadCell();
...
void LoadDataPlayer(int iClient)
{
	...
	char sQuery[1024];
	FormatEx(sQuery, sizeof(sQuery), "SELECT `weapon_knife`, `weapon_taser`, `weapon_inferno`, `weapon_hegrenade`, `weapon_glock`, `weapon_hkp2000`, `weapon_tec9`, `weapon_usp_silencer`, `weapon_p250`, `weapon_cz75a`, `weapon_fiveseven`, `weapon_elite`, `weapon_revolver`, `weapon_deagle`, `weapon_negev`, `weapon_m249`, `weapon_mag7`, `weapon_sawedoff`, `weapon_nova`, `weapon_xm1014`, `weapon_bizon`, `weapon_mac10`, `weapon_ump45`, `weapon_mp9`, `weapon_mp7`, `weapon_p90`, `weapon_galilar`, `weapon_famas`, `weapon_ak47`, `weapon_m4a1`, `weapon_m4a1_silencer`, `weapon_aug`, `weapon_sg556`, `weapon_ssg08`, `weapon_awp`, `weapon_scar20`, `weapon_g3sg1`, `weapon_usp`, `weapon_p228`, `weapon_m3`, `weapon_tmp`, `weapon_mp5navy`, `weapon_galil`, `weapon_scout`, `weapon_sg550`, `weapon_sg552`, `weapon_mp5sd` FROM `%s_weapons` WHERE `steam` = '%s';", g_sTableName, g_sSteamID[iClient]);
...
{
	char sQuery[2048], sSaveName[MAX_NAME_LENGTH * 2 + 1];
	g_hDatabase.Escape(GetFixNamePlayer(iClient), sSaveName, sizeof(sSaveName));

...
#define PLUGIN_NAME "[LR] Module - No World Damage"
...
public void OnPluginStart()
{
	...
	g_hCookie = RegClientCookie("LR_NoWorldDamage", "LR_NoWorldDamage", CookieAccess_Private);
	HookEvent("player_hurt", HookDamage, EventHookMode_Pre);
	LoadTranslations("lr_module_noworlddamage.phrases");
	ConfigLoad();
...
void ConfigLoad()
{
	...
	if(!sPath[0]) BuildPath(Path_SM, sPath, sizeof(sPath), "configs/levels_ranks/noworlddamage.ini");
	KeyValues hLR = new KeyValues("LR_NoWorldDamage");
...
public Action HookDamage(Event hEvent, const char[] sName, bool bDontBroadcast)
{
	...
	if(iClient && !iAttacker && LR_GetClientInfo(iClient, ST_RANK) >= g_iLevel && !g_bActive[iClient])
	{
...
void LR_OnMenuCreated(LR_MenuType OnMenuType, int iClient, Menu hMenu)
{
	...
	{
		FormatEx(sText, sizeof(sText), "%T", !g_bActive[iClient] ? "NoWorldDamage_On" : "NoWorldDamage_Off", iClient);
		hMenu.AddItem("NoWorldDamage", sText);
	}
	else
	{
		FormatEx(sText, sizeof(sText), "%T", "NoWorldDamage_Closed", iClient, g_iLevel);
		hMenu.AddItem("NoWorldDamage", sText, ITEMDRAW_DISABLED);
	}
}

void LR_OnMenuItemSelected(LR_MenuType OnMenuType, int iClient, const char[] sInfo)
{
	if(!strcmp(sInfo, "NoWorldDamage"))
	{
